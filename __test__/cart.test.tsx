...
describe("RTL Test", () => {
  ...
  it("장바구니에서 체크박스를 누르면 배송비를 고려하여 해당 금액이 반영된다.", async () => {
    ...
    const expectedDeliveryFeeText =
      deliveryFeeElement.textContent?.trim() || "";
    ...
    await waitFor(() => {
      expect(totalPurchasePrice.textContent).toContain(
  ...
  it("장바구니에서 상품을 삭제하면 해당 상품이 화면에서 사라진다.", async () => {
    ...
    const updatedCartItemCards = await screen.queryAllByTestId(
      "cart-item-card"
    );
