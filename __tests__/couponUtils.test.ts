...
import { isCouponDisabled } from '../src/components/Modal/utils/isCouponDisabled';
...
describe('calculateCouponDiscount()', () => {
  it('fixed 타입은 discount 값을 그대로 반환한다.', () => {
    ...
    expect(calculateCouponDiscount(c, 200_000, sampleItems, 3000)).toBe(5000);
  });

  it('freeShipping 타입은 deliveryFee를 반환한다.', () => {
    const c = coupons[2] as FreeShippingCoupon;
    expect(calculateCouponDiscount(c, 60_000, sampleItems, 6000)).toBe(6000);
  });

  it('percentage 타입은 주문금액 * (discount/100)을 내림한다.', () => {
    const c = coupons[3] as PercentageCoupon;
    expect(calculateCouponDiscount(c, 123_456, sampleItems, 0)).toBe(Math.floor(123_456 * 0.3));
  });

  it('buyXgetY 타입은 최고가 상품 가격 * getQuantity를 반환한다.', () => {
    const c = coupons[1] as BuyXGetYCoupon;
    expect(calculateCouponDiscount(c, 0, sampleItems, 0)).toBe(300_000_000 * 1);
  });

  it('items가 비어있으면 buyXgetY 는 0을 반환한다.', () => {
    const c = coupons[1] as BuyXGetYCoupon;
    expect(calculateCouponDiscount(c, 0, [], 0)).toBe(0);
  });
});
...
describe('isCouponDisabled()', () => {
  ...
  it('만료일이 지나면 비활성화된다.', () => {
    ...
    expect(isCouponDisabled(expired, 200_000, sampleItems, baseDate)).toBe(true);
  });

  it('fixed/무료배송은 최소 주문금액 미만이면 비활성화된다.', () => {
    expect(isCouponDisabled(coupons[0] as FixedCoupon, 90_000, [], baseDate)).toBe(true);
    expect(isCouponDisabled(coupons[2] as FreeShippingCoupon, 40_000, [], baseDate)).toBe(true);
  });

  it('fixed/무료배송은 최소 주문금액 이상이면 활성화된다.', () => {
    expect(isCouponDisabled(coupons[0] as FixedCoupon, 100_000, [], baseDate)).toBe(false);
    expect(isCouponDisabled(coupons[2] as FreeShippingCoupon, 50_000, [], baseDate)).toBe(false);
  });
  ...
  it('percentage 타입은 availableTime 외에는 비활성화된다.', () => {
    ...
    expect(isCouponDisabled(c, 200_000, sampleItems, before)).toBe(true);
    expect(isCouponDisabled(c, 200_000, sampleItems, after)).toBe(true);
    expect(isCouponDisabled(c, 200_000, sampleItems, baseDate)).toBe(false);
  });

  it('buyXgetY 타입은 (buy+get) 수량 미만이면 비활성화된다.', () => {
    const c = coupons[1] as BuyXGetYCoupon;
    const fewItems = [{ id: 1, quantity: 2, product: { price: 1000 } }] as CartItemType[];
    expect(isCouponDisabled(c, 0, fewItems, baseDate)).toBe(true);
  });

  it('buyXgetY 타입은 수량 조건을 만족하면 활성화된다.', () => {
    const c = coupons[1] as BuyXGetYCoupon;
    const enough = [{ id: 1, quantity: 3, product: { price: 1000 } }] as CartItemType[];
    expect(isCouponDisabled(c, 0, enough, baseDate)).toBe(false);
  });
});

describe('getBestCoupons()', () => {
  it('유효한 쿠폰 중 가장 큰 할인 2개를 반환한다 (시간 내).', () => {
    const best = getBestCoupons(coupons as Coupon[], 100_000, sampleItems, 0, baseDate);
  ...
  it('시간 외 MIRACLESALE 은 제외되고, 그 다음 2개를 반환한다.', () => {
    ...
    const best = getBestCoupons(coupons as Coupon[], 100_000, sampleItems, 0, offTime);
    expect(best.map((c) => c.code)).toEqual(['BOGO', 'FIXED5000']);
  });

  it('전체 쿠폰이 비활성화되면 빈 배열을 반환한다.', () => {
    const offTime = new Date(2025, 5, 15, 8, 0, 0);
    const best = getBestCoupons(coupons as Coupon[], 10_000, [], 3000, offTime);
    expect(best).toEqual([]);
