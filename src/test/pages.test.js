...
describe('페이지 기능 테스트', () => {
  beforeAll(async () => {
    ...
    db.productList.forEach((product, index) => firebase.firestore().collection(SCHEMA.PRODUCT).doc(index).set(product));
  ...
  describe('상품 목록 기능 테스트', () => {
    ...
    it('상품 목록에서 장바구니 아이콘을 클릭하면, 장바구니에 상품이 추가된다.', async () => {
      await act(async () => {
        const shoppingCartButtonList = await waitFor(() => screen.getAllByTestId('shopping-cart-icon'));
  ...
  describe('장바구니 기능 테스트', () => {
    ...
    it('장바구니에 상품이 추가되었는지 확인한다.', async () => {
      await act(async () => {
        ...
        const shoppingCartItem = (await firebase.firestore().collection(SCHEMA.SHOPPING_CART).get()).data;
