...
describe('cartReducer 작동 테스트 ', () => {
  test('장바구니 추가 요청을 보냈을 때에 해당 상품의 수량이 1인 채로 cartList에 새롭게 추가된다. ', () => {
    ...
    expect(result).toEqual({
      cartList: [{ id: 3, quantity: 1, isChecked: false }],
    });
  });
  ...
  test('장바구니에 있는 상품의 수량을 1씩 줄일 수 있다.', () => {
    const existCartList = {
      cartList: [
        { id: 3, quantity: 1, isChecked: false },
        { id: 1, quantity: 2, isChecked: false },
      ],
    };
    const result = cartReducer(existCartList, {
      type: MINUS_ITEM,
    ...
    expect(result).toEqual({
      cartList: [
        { id: 3, quantity: 1, isChecked: false },
        { id: 1, quantity: 1, isChecked: false },
      ],
    });
  });
  ...
  test('체크된 항목들을 장바구니에서 삭제할 수 있다', () => {
    const existCartList = {
      cartList: [
        ...
        { id: 2, quantity: 1, isChecked: false },
        { id: 4, quantity: 4, isChecked: true },
      ],
    };
    const result = cartReducer(existCartList, {
