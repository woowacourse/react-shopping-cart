...
import {
  putInSelectedCartItemIds,
  takeOutSelectedCartItemIds,
...
describe("selectedCartItemIdsState", () => {
  ...
  it("초기값 세팅", () => {
    ...
    const { result } = renderHook(
      () => useRecoilValue(selectedCartItemIdsState),
      {
        wrapper: RecoilRoot,
      }
    );
  ...
  it("스토리지와의 동기화", () => {
    const { result } = renderHook(
      () => useSetRecoilState(selectedCartItemIdsState),
      {
        wrapper: RecoilRoot,
      }
    );
    ...
    expect(mockPutInSelectedCartItemIds).toHaveBeenCalledWith(
