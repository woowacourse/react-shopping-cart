...
jest.mock("../api/cart", () => {
  ...
  return {
    getShoppingCartData: jest.fn(() => Promise.resolve(mockCartData)),
    addCartItem: jest.fn((productId: string) => {
      const foundProduct = mockProductList.find((p) => p.id === productId);
      if (!foundProduct) {
        throw new Error(`${productId} id를 가진 Product가 존재하지 않습니다.`);
      }
      mockCartData.push({
        id: String(mockCartData.length + 1),
        quantity: 1,
    ...
    deleteCartItem: jest.fn((cartId: string) => {
      const index = mockCartData.findIndex(item => item.id.toString() === cartId);
      if (index !== -1) {
    ...
    patchCartItem: jest.fn((cartId: string, quantity: number) => {
      const cartItem = mockCartData.find((item) => item.id === cartId);
      if (!cartItem) {
...
jest.mock("../api/baseAPI", () => ({
  getProductList: jest.fn(() => Promise.resolve([
    {
      id: "1",
      name: "상품1",
      price: 10000,
