...
const useFetch = <T>(url: string, method = 'GET'): [FetchState<T>, (body?: any) => Promise<void>] => {
  ...
  const fetchData = useCallback(
    async (body?: any) => {
      ...
      try {
        const response: Response = await fetch(url, { method, body: body ? JSON.stringify(body) : null, headers: { 'Content-Type': 'application/json' } });
        if (!response.ok) {
